...
veiculoano_bp = Blueprint("veiculoano", __name__)
...
def obter_codigos_wf(banco_homo):
    """Obtém todos os códigos da tabela VeiculoAno do banco homólogo"""
    try:
        if not banco_homo:
            return []
        ...
        cursor.execute("SELECT VeiculoAno_Codigo FROM VeiculoAno")
...
def obter_exibicao_wf(banco_homo, codigo):
    """Obtém a exibição de um código específico da tabela VeiculoAno do banco homólogo"""
    try:
        ...
        cursor = conexao.cursor()
        cursor.execute("""
            SELECT VeiculoAno_Fabricacao, VeiculoAno_Modelo 
            FROM VeiculoAno 
            WHERE VeiculoAno_Codigo = ?
        """, (codigo,))
        resultado = cursor.fetchone()
        ...
        logger.error(f"Erro ao obter exibição WF para código {codigo}: {str(e)}")
        return None

def atualizar_exibicoes_apos_importacao(banco_usuario, banco_homo):
    """Atualiza automaticamente as exibições após importação baseado nos códigos WF"""
    try:
        if not banco_homo:
            return
        ...
        if not conexao:
            logger.error("Falha ao conectar para atualizar exibições")
            return
        
        cursor = conexao.cursor()
        
        cursor.execute("""
            SELECT id, VeiculoAno_Codigo, VeiculoAno_Exibicao 
            FROM VeiculoAno_DePara 
            WHERE VeiculoAno_Codigo IS NOT NULL AND VeiculoAno_Codigo != 'S/DePara'
        """)
        registros = cursor.fetchall()
        
        atualizacoes = 0
        for registro in registros:
            id_registro, codigo_wf, exibicao_atual = registro
            exibicao_wf = obter_exibicao_wf(banco_homo, codigo_wf)
            
            if exibicao_wf and exibicao_wf != exibicao_atual:
                cursor.execute("""
                    UPDATE VeiculoAno_DePara 
                    SET VeiculoAno_Exibicao = ? 
                    WHERE id = ?
                """, (exibicao_wf, id_registro))
                atualizacoes += 1
                logger.info(f"Exibição atualizada para código {codigo_wf}: {exibicao_wf}")
        
        conexao.commit()
        cursor.close()
        conexao.close()
        
        logger.info(f"Atualizações automáticas de exibição: {atualizacoes} registros")
        
    except Exception as e:
        logger.error(f"Erro ao atualizar exibições após importação: {str(e)}")
...
def index():
    try:
        ...
        if not conexao:
            ...
            return render_template('veiculoano.html', 
                                 registros=[], 
                                 colunas=[], 
        ...
        cursor = conexao.cursor()
        cursor.execute("SELECT * FROM VeiculoAno_DePara")
        registros = cursor.fetchall()
        ...
        logger.error(f"Erro em veiculoano: {str(e)}")
...
def exportar():
    try:
        ...
        if not banco_usuario:
            flash('Banco não configurado para este projeto.', 'error')
            return redirect(url_for('veiculoano.index'))
        ...
        cursor = conexao.cursor()
        cursor.execute("""
            SELECT id, ve_fabmod, VeiculoAno_Codigo, VeiculoAno_Exibicao
            FROM VeiculoAno_DePara
        """)
        registros = cursor.fetchall()
        ...
        mapeamento_colunas = {
            'id': 'ID',
            've_fabmod': 'Código de Origem',
            'VeiculoAno_Codigo': 'VeiculoAno_Codigo',
            'VeiculoAno_Exibicao': 'VeiculoAno_Exibicao'
        }
        ...
        ws = wb.create_sheet(title="VeiculoAno_DePara")
        ...
        for row_num, registro in enumerate(registros, 2):
            for col_num, valor in enumerate(registro, 1):
                ...
                if colunas_originais[col_num-1] == 'VeiculoAno_Codigo':
        ...
        nome_arquivo = "VeiculoAno_DePara.xlsx"
        return send_file(
            output,
            as_attachment=True,
        ...
        logger.error(f"Erro ao exportar veiculoano: {str(e)}")
        flash(f'Erro na exportação: {str(e)}', 'error')
        return redirect(url_for('veiculoano.index'))

@veiculoano_bp.route('/exportar_filtrados', methods=['POST'])
def exportar_filtrados():
    try:
        ...
        ws = wb.create_sheet(title="VeiculoAno_Filtrado")
        ...
        return send_file(
            output,
            as_attachment=True,
            download_name="VeiculoAno_Filtrado.xlsx",
            mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        )
        
    except Exception as e:
        logger.error(f"Erro ao exportar veiculoano filtrada: {str(e)}")
        return jsonify({'success': False, 'message': f'Erro na exportação: {str(e)}'}), 500

@veiculoano_bp.route('/export_wf')
def export_wf():
    """Exporta a tabela VeiculoAno do banco homólogo (BancoHomo)"""
    try:
        if 'projeto_selecionado' not in session:
            flash('Nenhum projeto selecionado.', 'error')
            return redirect(url_for('auth.selecionar_projeto'))
        ...
        cursor = conexao.cursor()
        cursor.execute("SELECT VeiculoAno_Codigo, VeiculoAno_Fabricacao, VeiculoAno_Modelo, VeiculoAno_Ativo FROM VeiculoAno")
        registros = cursor.fetchall()
        colunas = [column[0] for column in cursor.description]
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='VeiculoAno_WF', index=False)
        
        output.seek(0)
        
        nome_arquivo = "VeiculoAno_WF.xlsx"
...
@veiculoano_bp.route('/importar', methods=['POST'])
def importar():
    try:
        ...
        mapeamento_colunas = {
            'ID': 'id',
            'Código de Origem': 've_fabmod',
            'VeiculoAno_Codigo': 'VeiculoAno_Codigo',
            'VeiculoAno_Exibicao': 'VeiculoAno_Exibicao'
        }
        
        colunas_banco = ['id', 've_fabmod', 'VeiculoAno_Codigo', 'VeiculoAno_Exibicao']
        
        # Verificar colunas obrigatórias
        colunas_obrigatorias = ['ID', 'Código de Origem', 'VeiculoAno_Codigo']
        ...
        cursor.execute("SELECT COUNT(*) FROM VeiculoAno_DePara")
        ...
        try:
            ...
            registros_codigo_vazio = []  # Para coletar IDs com Código de Origem vazio
            
            for registro in registros_mapeados:
                registro_id = registro.get('id')
                codigo_origem = registro.get('ve_fabmod')
                veiculoano_codigo = registro.get('VeiculoAno_Codigo')
                
                if not registro_id:
                    continue
                
                # Verificar se Código de Origem está vazio
                if not codigo_origem:
                    registros_codigo_vazio.append(registro_id)
                    logger.warning(f"AVISO: Registro ID {registro_id} tem 'Código de Origem' vazio. Atualização limitada.")
                
                # DEBUG: Log detalhado de cada registro
                logger.info(f"Processando registro ID {registro_id}:")
                logger.info(f"  - ve_fabmod: {codigo_origem}")
                logger.info(f"  - VeiculoAno_Codigo: {veiculoano_codigo}")
                
                # Verificar se o registro existe
                cursor.execute("SELECT id, ve_fabmod, VeiculoAno_Codigo FROM VeiculoAno_DePara WHERE id = ?", (registro_id,))
                resultado = cursor.fetchone()
                ...
                if resultado:
                    ...
                    # Se Código de Origem está vazio, manter o valor atual do banco
                    if not codigo_origem:
                        cursor.execute("""
                            UPDATE VeiculoAno_DePara 
                            SET VeiculoAno_Codigo = ?, 
                                VeiculoAno_Exibicao = ?
                            WHERE id = ?
                        """, (
                            veiculoano_codigo,
                            registro.get('VeiculoAno_Exibicao'),
                            registro_id
                        ))
                    else:
                        # Código de Origem preenchido - atualizar todos os campos
                        cursor.execute("""
                            UPDATE VeiculoAno_DePara 
                            SET ve_fabmod = ?, 
                                VeiculoAno_Codigo = ?, 
                    ...
                    # Para INSERT, Código de Origem é obrigatório
                    if not codigo_origem:
                        logger.error(f"ERRO: Não é possível inserir registro ID {registro_id} sem 'Código de Origem'")
                        continue
                    
                    cursor.execute("""
                        INSERT INTO VeiculoAno_DePara 
                        (id, ve_fabmod, VeiculoAno_Codigo, VeiculoAno_Exibicao)
                        VALUES (?, ?, ?, ?)
                    """, (
                        registro_id,
                        codigo_origem,
            ...
            if registros_codigo_vazio:
                mensagem_base += f' AVISO: Os registros com ID {", ".join(map(str, registros_codigo_vazio))} possuem "Código de Origem" vazio. Para esses registros, apenas os demais campos foram atualizados. Para corrigir o "Código de Origem", edite diretamente na tela.'
                logger.warning(f"Registros com Código de Origem vazio: {registros_codigo_vazio}")
            
            logger.info("Executando COMMIT...")
            conexao.commit()
            logger.info("COMMIT executado com sucesso")
            
            # Atualizar exibições com base no banco homólogo
            banco_homo = obter_banco_homo(projeto_id)
            if banco_homo:
                logger.info("Atualizando exibições com base no banco homólogo...")
                atualizar_exibicoes_apos_importacao(banco_usuario, banco_homo)
        ...
        cursor.execute("SELECT TOP 5 id, ve_fabmod, VeiculoAno_Codigo, VeiculoAno_Exibicao FROM VeiculoAno_DePara ORDER BY id")
        ...
        logger.error(f"Erro ao importar veiculoano: {str(e)}")
        import traceback
...
@veiculoano_bp.route('/update', methods=['POST'])
def update_registro():
    """Endpoint para atualizar um registro individual via edição inline"""
    logger.info("=== UPDATE REGISTRO VEICULOANO ENDPOINT ACESSADO ===")
    ...
    try:
        ...
        colunas_permitidas = ['VeiculoAno_Codigo', 'VeiculoAno_Exibicao']
        ...
        try:
            cursor.execute(f"SELECT TOP 1 {field} FROM VeiculoAno_DePara WHERE id = ?", (record_id,))
            resultado = cursor.fetchone()
            if not resultado:
        ...
        query = f"UPDATE VeiculoAno_DePara SET {field} = ? WHERE id = ?"
...
@veiculoano_bp.route('/update_batch', methods=['POST'])
def update_batch():
...
@veiculoano_bp.route('/get_exibicao_wf/<codigo>')
def get_exibicao_wf(codigo):
    """Endpoint para obter a exibição de um código da base WF"""
    try:
        if 'projeto_selecionado' not in session:
            return jsonify({'success': False, 'message': 'Nenhum projeto selecionado'})
        ...
        exibicao = obter_exibicao_wf(banco_homo, codigo)
        ...
        logger.error(f"Erro ao buscar exibição WF: {str(e)}")
        return jsonify({'success': False, 'message': f'Erro ao buscar exibição: {str(e)}'})
...
def dados_veiculoano(banco_usuario):
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM VeiculoAno_DePara WHERE VeiculoAno_Codigo = 'S/DePara'"
        )
        ...
        logger.error(f"Erro ao calcular dados VeiculoAno_DePara: {e}")
        return {"qtd": 0, "qtdPendente": 0, "percentualConclusao": 0}
    finally:
        if conexao:
            conexao.close()