...
centroresultado_bp = Blueprint("centroresultado", __name__)
...
def obter_codigos_wf(banco_homo):
    """Obtém todos os códigos da tabela CentroResultado do banco homólogo"""
    try:
        if not banco_homo:
            return []
        ...
        cursor.execute("SELECT CentroResultado_Codigo FROM CentroResultado")
...
def obter_descricao_wf(banco_homo, codigo):
    """Obtém a descrição de um código específico da tabela CentroResultado do banco homólogo"""
    try:
        ...
        cursor = conexao.cursor()
        cursor.execute("SELECT CentroResultado_Descricao FROM CentroResultado WHERE CentroResultado_Codigo = ?", (codigo,))
        resultado = cursor.fetchone()
...
def atualizar_descricoes_apos_importacao(banco_usuario, banco_homo):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, CentroResultado_Codigo, Centroresultado_Descricao 
            FROM CentroResultado_DePara 
            WHERE CentroResultado_Codigo IS NOT NULL AND CentroResultado_Codigo != 'S/DePara'
        """)
        registros = cursor.fetchall()
        ...
        for registro in registros:
            ...
            if descricao_wf and descricao_wf != descricao_atual:
                cursor.execute("""
                    UPDATE CentroResultado_DePara 
                    SET Centroresultado_Descricao = ? 
                    WHERE id = ?
...
def index():
    try:
        ...
        if not conexao:
            ...
            return render_template('centroresultado.html', 
                                 registros=[], 
                                 colunas=[], 
        ...
        cursor = conexao.cursor()
        cursor.execute("SELECT * FROM CentroResultado_DePara")
        registros = cursor.fetchall()
        ...
        logger.error(f"Erro em centroresultado: {str(e)}")
...
@centroresultado_bp.route('/exportar')
def exportar():
    try:
        if 'projeto_selecionado' not in session:
            flash('Nenhum projeto selecionado.', 'error')
        ...
        if not banco_usuario:
            flash('Banco não configurado para este projeto.', 'error')
            return redirect(url_for('centroresultado.index'))
        ...
        cursor = conexao.cursor()
        cursor.execute("""
            SELECT id, cdbdados, cdccusto, noccusto, dcccusto, sgccusto,
                   dtbloqueio, tbempresa, cdccresp, tpccusto, tbfiliais,
                   CentroResultado_Codigo, Centroresultado_Descricao, Estrutura_Codigo
            FROM CentroResultado_DePara
        """)
        registros = cursor.fetchall()
        ...
        mapeamento_colunas = {
            'id': 'ID',
            'cdbdados': 'Código Banco Dados',
            'cdccusto': 'Código Centro Custo',
            'noccusto': 'Nome Centro Custo',
            'dcccusto': 'Descrição Centro Custo',
            'sgccusto': 'Sigla Centro Custo',
            'dtbloqueio': 'Data Bloqueio',
            'tbempresa': 'Empresa',
            'cdccresp': 'Código Centro Responsabilidade',
            'tpccusto': 'Tipo Centro Custo',
            'tbfiliais': 'Filiais',
            'CentroResultado_Codigo': 'CentroResultado_Codigo',
            'Centroresultado_Descricao': 'Centroresultado_Descricao',
        ...
        ws = wb.create_sheet(title="CentroResultado_DePara")
        ...
        for row_num, registro in enumerate(registros, 2):
            for col_num, valor in enumerate(registro, 1):
                ...
                if col_num == 12:  # CentroResultado_Codigo
        ...
        nome_arquivo = "CentroResultado_DePara.xlsx"
        return send_file(
            output,
            as_attachment=True,
        ...
        logger.error(f"Erro ao exportar centroresultado: {str(e)}")
        flash(f'Erro na exportação: {str(e)}', 'error')
        return redirect(url_for('centroresultado.index'))

@centroresultado_bp.route('/exportar_filtrados', methods=['POST'])
def exportar_filtrados():
    try:
        ...
        ws = wb.create_sheet(title="CentroResultado_Filtrado")
        ...
        for row_num, registro in enumerate(registros_filtrados, 2):
            for col_num, header in enumerate(headers, 1):
                ...
                if header == 'CentroResultado_Codigo':
                    if not valor or valor == '':
                        cell.fill = laranja
                    elif valor == 'S/DePara':
                        cell.fill = amarelo
        ...
        return send_file(
            output,
            as_attachment=True,
            download_name="CentroResultado_Filtrado.xlsx",
            mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        )
        
    except Exception as e:
        logger.error(f"Erro ao exportar centroresultado filtrada: {str(e)}")
        return jsonify({'success': False, 'message': f'Erro na exportação: {str(e)}'}), 500

@centroresultado_bp.route('/export_wf')
def export_wf():
    """Exporta a tabela CentroResultado do banco homólogo (BancoHomo)"""
    try:
        if 'projeto_selecionado' not in session:
            flash('Nenhum projeto selecionado.', 'error')
            return redirect(url_for('auth.selecionar_projeto'))
        ...
        cursor = conexao.cursor()
        cursor.execute("SELECT CentroResultado_Codigo, CentroResultado_Descricao, CentroResultado_Ativo FROM CentroResultado")
        registros = cursor.fetchall()
        colunas = [column[0] for column in cursor.description]
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='CentroResultado_WF', index=False)
        
        output.seek(0)
        
        nome_arquivo = "CentroResultado_WF.xlsx"
...
@centroresultado_bp.route('/importar', methods=['POST'])
def importar():
    try:
        ...
        # Mapeamento das colunas - ACEITAR TANTO NOMES AMIGÁVEIS QUANTO NOMES ORIGINAIS
        mapeamento_colunas = {
            # Nomes amigáveis -> Nomes do banco
            'ID': 'id',
            'Código Banco Dados': 'cdbdados',
            'Código Centro Custo': 'cdccusto',
            'Nome Centro Custo': 'noccusto',
            'Descrição Centro Custo': 'dcccusto',
            'Sigla Centro Custo': 'sgccusto',
            'Data Bloqueio': 'dtbloqueio',
            'Empresa': 'tbempresa',
            'Código Centro Responsabilidade': 'cdccresp',
            'Tipo Centro Custo': 'tpccusto',
            'Filiais': 'tbfiliais',
            'CentroResultado_Codigo': 'CentroResultado_Codigo',
            'Centroresultado_Descricao': 'Centroresultado_Descricao',
            'Estrutura_Codigo': 'Estrutura_Codigo',
            # Nomes originais do banco -> Nomes do banco (para compatibilidade)
            'id': 'id',
            'cdbdados': 'cdbdados',
            'cdccusto': 'cdccusto',
            'noccusto': 'noccusto',
            'dcccusto': 'dcccusto',
            'sgccusto': 'sgccusto',
            'dtbloqueio': 'dtbloqueio',
            'tbempresa': 'tbempresa',
            'cdccresp': 'cdccresp',
            'tpccusto': 'tpccusto',
            'tbfiliais': 'tbfiliais',
            'Timestamp': 'cdbdados',  # Mapear colunas extras se necessário
            'Description': 'noccusto',
            'Marca Temporal': 'dtbloqueio',
            'Descrição': 'Centroresultado_Descricao'
        }
        
        colunas_banco = ['id', 'cdbdados', 'cdccusto', 'noccusto', 'dcccusto', 'sgccusto',
                        'dtbloqueio', 'tbempresa', 'cdccresp', 'tpccusto', 'tbfiliais',
                        'CentroResultado_Codigo', 'Centroresultado_Descricao', 'Estrutura_Codigo']
        
        # Verificar colunas obrigatórias - ACEITAR TANTO NOMES AMIGÁVEIS QUANTO ORIGINAIS
        colunas_obrigatorias = ['id', 'cdccusto', 'CentroResultado_Codigo']
        colunas_obrigatorias_encontradas = []
        colunas_faltantes = []
        
        for col_obrigatoria in colunas_obrigatorias:
            encontrou = False
            # Buscar por nomes amigáveis
            for chave_planilha, col_banco in mapeamento_colunas.items():
                if col_banco == col_obrigatoria and chave_planilha in colunas_excel:
                    colunas_obrigatorias_encontradas.append(chave_planilha)
                    encontrou = True
                    break
            
            if not encontrou:
                # Buscar pelo nome original do banco
                if col_obrigatoria in colunas_excel:
                    colunas_obrigatorias_encontradas.append(col_obrigatoria)
                    encontrou = True
            
            if not encontrou:
                colunas_faltantes.append(col_obrigatoria)
        
        if colunas_faltantes:
            # Traduzir nomes para amigáveis para a mensagem de erro
            nomes_amigaveis = {
                'id': 'ID',
                'cdccusto': 'Código Centro Custo',
                'CentroResultado_Codigo': 'CentroResultado_Codigo'
            }
            colunas_faltantes_amigaveis = [nomes_amigaveis.get(col, col) for col in colunas_faltantes]
            
            # CORREÇÃO: Garantir que todos os elementos sejam strings antes do join
            colunas_faltantes_str = [str(col) for col in colunas_faltantes_amigaveis]
            
            return jsonify({
                'success': False, 
                'message': f'Colunas obrigatórias faltando no arquivo: {", ".join(colunas_faltantes_str)}'
            })
        
        # Resto do código permanece igual...
        # [Manter todo o resto do código de importação]
        ...
        for registro in registros:
            ...
            for coluna_banco in colunas_banco:
                ...
                for chave_planilha, col_banco_mapeada in mapeamento_colunas.items():
                    if col_banco_mapeada == coluna_banco and chave_planilha in colunas_excel:
                        valor = registro.get(chave_planilha)
                        if valor is not None:
                            break
                
                # Se não encontrou pelo mapeamento, tentar pelo nome original
                if valor is None and coluna_banco in colunas_excel:
                    valor = registro.get(coluna_banco)
        ...
        cursor.execute("SELECT COUNT(*) FROM CentroResultado_DePara")
        ...
        try:
            ...
            registros_codigo_vazio = []  # Para coletar IDs com Código Centro Custo vazio
            
            for registro in registros_mapeados:
                registro_id = registro.get('id')
                codigo_centro_custo = registro.get('cdccusto')
                centro_resultado_codigo = registro.get('CentroResultado_Codigo')
                
                if not registro_id:
                    continue
                
                # Verificar se Código Centro Custo está vazio
                if not codigo_centro_custo:
                    registros_codigo_vazio.append(registro_id)
                    logger.warning(f"AVISO: Registro ID {registro_id} tem 'Código Centro Custo' vazio. Atualização limitada.")
                
                # DEBUG: Log detalhado de cada registro
                logger.info(f"Processando registro ID {registro_id}:")
                logger.info(f"  - cdccusto: {codigo_centro_custo}")
                logger.info(f"  - CentroResultado_Codigo: {centro_resultado_codigo}")
                
                # Verificar se o registro existe
                cursor.execute("SELECT id, cdccusto, CentroResultado_Codigo FROM CentroResultado_DePara WHERE id = ?", (registro_id,))
                resultado = cursor.fetchone()
                ...
                if resultado:
                    ...
                    # Se Código Centro Custo está vazio, manter o valor atual do banco
                    if not codigo_centro_custo:
                        cursor.execute("""
                            UPDATE CentroResultado_DePara 
                            SET cdbdados = ?,
                                ...
                                tpccusto = ?,
                                tbfiliais = ?,
                                CentroResultado_Codigo = ?, 
                                Centroresultado_Descricao = ?, 
                                Estrutura_Codigo = ?
                            WHERE id = ?
                        """, (
                            registro.get('cdbdados'),
                            registro.get('noccusto'),
                            registro.get('dcccusto'),
                            registro.get('sgccusto'),
                            registro.get('dtbloqueio'),
                            registro.get('tbempresa'),
                            registro.get('cdccresp'),
                            registro.get('tpccusto'),
                            registro.get('tbfiliais'),
                            centro_resultado_codigo,
                            registro.get('Centroresultado_Descricao'),
                            registro.get('Estrutura_Codigo'),
                            registro_id
                        ))
                    else:
                        # Código Centro Custo preenchido - atualizar todos os campos
                        cursor.execute("""
                            UPDATE CentroResultado_DePara 
                            SET cdbdados = ?,
                                cdccusto = ?,
                    ...
                    # Para INSERT, Código Centro Custo é obrigatório
                    if not codigo_centro_custo:
                        logger.error(f"ERRO: Não é possível inserir registro ID {registro_id} sem 'Código Centro Custo'")
                        continue
                    
                    cursor.execute("""
                        INSERT INTO CentroResultado_DePara 
                        (id, cdbdados, cdccusto, noccusto, dcccusto, sgccusto,
                         dtbloqueio, tbempresa, cdccresp, tpccusto, tbfiliais,
                         CentroResultado_Codigo, Centroresultado_Descricao, Estrutura_Codigo)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
            ...
            if registros_codigo_vazio:
                # CORREÇÃO: Garantir que todos os IDs sejam strings antes do join
                registros_codigo_vazio_str = [str(reg_id) for reg_id in registros_codigo_vazio]
                mensagem_base += f' AVISO: Os registros com ID {", ".join(registros_codigo_vazio_str)} possuem "Código Centro Custo" vazio. Para esses registros, apenas os demais campos foram atualizados. Para corrigir o "Código Centro Custo", edite diretamente na tela.'
                logger.warning(f"Registros com Código Centro Custo vazio: {registros_codigo_vazio}")
        ...
        cursor.execute("SELECT TOP 5 id, cdccusto, CentroResultado_Codigo, Centroresultado_Descricao FROM CentroResultado_DePara ORDER BY id")
        ...
        logger.error(f"Erro ao importar centroresultado: {str(e)}")
        import traceback
...
@centroresultado_bp.route('/update', methods=['POST'])
def update_registro():
    """Endpoint para atualizar um registro individual via edição inline"""
    logger.info("=== UPDATE REGISTRO CENTRORESULTADO ENDPOINT ACESSADO ===")
    ...
    try:
        ...
        colunas_permitidas = ['CentroResultado_Codigo', 'Centroresultado_Descricao', 'Estrutura_Codigo']
        ...
        try:
            cursor.execute(f"SELECT TOP 1 {field} FROM CentroResultado_DePara WHERE id = ?", (record_id,))
            resultado = cursor.fetchone()
            if not resultado:
        ...
        query = f"UPDATE CentroResultado_DePara SET {field} = ? WHERE id = ?"
...
@centroresultado_bp.route('/update_batch', methods=['POST'])
def update_batch():
...
@centroresultado_bp.route('/get_descricao_wf/<codigo>')
...
def dados_centroresultado(banco_usuario):
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM CentroResultado_DePara WHERE CentroResultado_Codigo = 'S/DePara'"
        )
        ...
        logger.error(f"Erro ao calcular dados CentroResultado_DePara: {e}")
        return {"qtd": 0, "qtdPendente": 0, "percentualConclusao": 0}
    finally:
        if conexao:
            conexao.close()